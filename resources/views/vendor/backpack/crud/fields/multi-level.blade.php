...
{{--    <label>{{ $field['label'] }}</label>--}}
    ...
    <div class="row" >
        ...
        <?php
        $items = $primary_dependency['model']::all()->pluck('name');
        $departments = collect([]);
        foreach($items as $item) {
            $department = explode('__', $item);
            $departments->push($department[0]);
        }
        $departments = $departments->unique();
        ?>

        @foreach($departments as $department)
            <div class="col-md-12"><strong>{{ucfirst($department)}}</strong></div>
            @foreach ($primary_dependency['model']::all() as $connected_entity_entry)
                @if(starts_with($connected_entity_entry['name'], $department))
                <div class="col-sm-{{ isset($primary_dependency['number_columns']) ? intval(12/$primary_dependency['number_columns']) : '4'}}">
                    <div class="checkbox">
                        <label>
                            ...
                            {{ ucwords(str_replace([$department.'__', '_'], ['', ' '],$connected_entity_entry->{$primary_dependency['attribute']})) }}
